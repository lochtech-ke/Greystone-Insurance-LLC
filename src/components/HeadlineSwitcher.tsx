...
export const HeadlineSwitcher: React.FC<HeadlineSwitcherProps> = ({ onOpenUnderwritingModal, onNavigate }) => {
  ...
  return (
    <div className="relative pt-32 pb-20 md:pt-40 md:pb-28 bg-gradient-hero overflow-hidden border-b border-[var(--bronze-500)]/20">
      
      <div className="absolute inset-0 bg-grid-pattern opacity-30 pointer-events-none" />

      
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-[var(--bronze-500)]/5 blur-[120px] rounded-full pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        
        <div className="flex flex-wrap items-center justify-center gap-2 mb-8 animate-fade-in">
          <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mr-2 flex items-center gap-1">
            <Layers className="w-3.5 h-3.5 text-[var(--bronze-500)]" /> Positioning Variant:
          </span>
          {headlineVariants.map((variant, idx) => (
            <button
              key={idx}
              onClick={() => setActiveVariant(idx)}
              className={`px-3 py-1.5 rounded text-[10px] uppercase tracking-wider font-semibold transition-all duration-200 cursor-pointer border ${
                activeVariant === idx
                  ? 'bg-[var(--bronze-500)] text-[var(--bg-dark-950)] border-[var(--bronze-200)] shadow-md font-bold'
                  : 'bg-[var(--bg-dark-850)]/80 text-slate-300 border-[var(--bg-dark-700)] hover:border-[var(--bronze-500)]/50 hover:text-white'
              }`}
            >
        ...
        <div className="text-center max-w-4xl mx-auto space-y-6 animate-fade-in-up" key={activeVariant}>
          
          
          <div className="inline-flex items-center gap-2 bg-[var(--bg-dark-800)]/90 border border-[var(--bronze-500)]/40 px-4 py-1.5 rounded-full text-[10px] font-bold tracking-widest text-[var(--bronze-500)] uppercase shadow-lg">
            <ShieldCheck className="w-4 h-4 text-[var(--bronze-500)]" />
          ...
          <h1 className="font-serif-display text-3xl sm:text-5xl lg:text-6xl font-bold text-white tracking-tight leading-[1.15]">
            ...
            <span className="text-gradient-bronze font-cinzel underline decoration-[var(--bronze-500)]/30 underline-offset-8">
              {current.highlight}
            </span>
          ...
          <p className="text-slate-300 text-lg sm:text-xl font-light leading-relaxed max-w-3xl mx-auto pt-2">
            {current.desc}
          </p>

          
          <div className="w-32 h-[2px] bg-gradient-to-r from-transparent via-[var(--bronze-500)] to-transparent mx-auto my-6" />

          
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <button
              onClick={onOpenUnderwritingModal}
              className="btn-primary-bronze w-full sm:w-auto px-8 py-4 text-xs tracking-widest uppercase shadow-2xl group cursor-pointer"
            >
              <FileCheck className="w-5 h-5 text-[var(--bg-dark-950)]" />
            ...
            >
              ...
              <ChevronRight className="w-5 h-5 text-[var(--bronze-500)] group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
        ...
        <div className="mt-16 pt-10 border-t border-[var(--bg-dark-700)] grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <div className="p-4 rounded-lg bg-[var(--bg-dark-850)]/60 border border-[var(--bg-dark-800)]">
            <div className="font-cinzel text-2xl sm:text-3xl font-extrabold text-[var(--bronze-500)]">$4.8B+</div>
            <div className="text-xs text-slate-400 font-medium uppercase tracking-wider mt-1">Risk Capital Facilitated</div>
          </div>
          <div className="p-4 rounded-lg bg-[var(--bg-dark-850)]/60 border border-[var(--bg-dark-800)]">
            <div className="font-cinzel text-2xl sm:text-3xl font-extrabold text-[var(--bronze-500)]">A+ Rating</div>
            <div className="text-xs text-slate-400 font-medium uppercase tracking-wider mt-1">Underwriting Syndicate Alignment</div>
          </div>
          <div className="p-4 rounded-lg bg-[var(--bg-dark-850)]/60 border border-[var(--bg-dark-800)]">
            <div className="font-cinzel text-2xl sm:text-3xl font-extrabold text-[var(--bronze-500)]">100%</div>
            <div className="text-xs text-slate-400 font-medium uppercase tracking-wider mt-1">Bank Covenant Acceptance</div>
          </div>
          <div className="p-4 rounded-lg bg-[var(--bg-dark-850)]/60 border border-[var(--bg-dark-800)]">
            <div className="font-cinzel text-2xl sm:text-3xl font-extrabold text-[var(--bronze-500)]">48 Hours</div>
