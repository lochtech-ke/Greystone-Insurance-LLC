...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  'Underwriting': 'text-[var(--bronze-500)] bg-[#D4AF37]/10 border-[var(--bronze-500)]/30',
};
  ...
  return (
    <main className="pt-24">
      ...
      <section className="py-20 bg-[var(--bg-dark-900)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-7">
            {ARTICLES.map((article) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border ${CATEGORY_COLORS[article.category] ?? 'text-slate-400 bg-white/5 border-[var(--bg-dark-700)]'}`}>
                    {article.category}
                  </span>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-[var(--bg-dark-700)]">
                  <div>
                    <div className="flex items-center gap-1.5 text-xs text-slate-300">
                      <User className="w-3.5 h-3.5 text-[var(--bronze-500)]" />
            ...
            {/* Coming Soon Card */}
            <div className="glass-panel rounded-xl p-7 space-y-4 border-dashed border-[var(--bg-dark-700)] opacity-60">
              <div className="w-12 h-12 rounded-xl bg-white/5 border border-[var(--bg-dark-700)] flex items-center justify-center">
      ...
      {selectedArticle && (
        ...
        >
          <div className="w-full max-w-3xl glass-panel rounded-2xl border border-[var(--bronze-500)]/40 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-fade-in">
            ...
            <div className="px-7 py-5 border-b border-[var(--bg-dark-700)] bg-[var(--bg-dark-850)] flex items-start justify-between relative">
              ...
              <div className="space-y-2 pr-4">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-400">
                  <span className="flex items-center gap-1"><User className="w-3.5 h-3.5 text-[var(--bronze-500)]" /> {selectedArticle.author} — {selectedArticle.authorTitle}</span>
                  <span>•</span>
            ...
            <div className="flex-1 overflow-y-auto px-7 py-7">
              <div className="prose prose-sm prose-invert max-w-none space-y-4">
                {selectedArticle.contentMarkdown.trim().split('\n').map((line, idx) => {
                  ...
                  if (line.startsWith('> ')) return <blockquote key={idx} className="border-l-4 border-[var(--bronze-500)] pl-4 py-1 bg-[var(--bg-dark-800)] rounded-r text-slate-200 text-sm italic my-4">{line.slice(2)}</blockquote>;
                  if (line.startsWith('---')) return <div key={idx} className="bronze-accent-line my-6" />;
                  if (line.startsWith('- ')) return <div key={idx} className="flex items-start gap-2 my-1"><span className="text-[var(--bronze-500)] mt-0.5">•</span><span className="text-slate-300 text-sm leading-relaxed">{line.slice(2)}</span></div>;
                  if (line.startsWith('|')) {
                    const cells = line.split('|').filter(Boolean).map(c => c.trim());
                    const isHeader = selectedArticle.contentMarkdown.split('\n')[idx + 1]?.startsWith('|---');
                    if (isHeader) return <tr key={idx}>{cells.map((c, ci) => <th key={ci} className="py-2 px-3 text-left text-xs font-bold text-[var(--bronze-500)] uppercase tracking-wider border-b border-[var(--bg-dark-700)]">{c}</th>)}</tr>;
                    if (line.startsWith('|---')) return null;
                    return <tr key={idx}>{cells.map((c, ci) => <td key={ci} className="py-2 px-3 text-sm text-slate-300 border-b border-[var(--bg-dark-800)]">{c}</td>)}</tr>;
                  }
            ...
            <div className="px-7 py-4 border-t border-[var(--bg-dark-700)] bg-[var(--bg-dark-900)]">
